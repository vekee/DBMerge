...
public class MergeThread extends Thread {
	...
	private void merge() throws SQLException, IOException, ClassNotFoundException {
			...
			if (compare(new String(newKeyValues), new String(oldKeyValues)) > 0) {
				...
				for (int i = 0; i < mergeTableInfo.getNewTableMergeColumnList().size(); i++) {
					...
					if (notEqual(newColumnValue, oldColumnValue)) {
						outputDiffColumn(newRs, mergeTableInfo.getNewTableMergeColumnList().get(i), newColumnValue,
								mergeTableInfo.getOldTableMergeColumnList().get(i), oldColumnValue);
					}
	...
	private void outputNewOnly(ResultSet newRs) throws SQLException, IOException {
		...
		synchronized (lock) {
			newOnlyOutBuffer.write(new String(stringBuilder).replaceFirst(",", "").getBytes(StandardCharsets.UTF_8));
			newOnlyOutBuffer.write(System.getProperty("lines.separator").getBytes());
	...
	private void outputOldOnly(ResultSet oldRs) throws SQLException, IOException {
		...
		stringBuilder.append("]");
		synchronized (lock) {
			oldOnlyOutBuffer.write(new String(stringBuilder).replaceFirst(",", "").getBytes(StandardCharsets.UTF_8));
	...
	private void outputDiffColumn(ResultSet newRs, String newColumn, String newColumnValue, String oldColumn,
			String oldColumnValue) throws SQLException, IOException {
		StringBuilder stringBuilder = new StringBuilder("mergeKey[");
		for (String newKeyColumn : mergeTableInfo.getNewTableKeyColumnList()) {
			stringBuilder.append(",");
			stringBuilder.append(newKeyColumn);
		...
		synchronized (lock) {
			diffColumnOutBuffer.write(new String(stringBuilder).replaceFirst(",", "").getBytes(StandardCharsets.UTF_8));
